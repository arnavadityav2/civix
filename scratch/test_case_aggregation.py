...
async def test_case_aggregation():
    async with engine.connect() as conn:
        # Check specific cases
        target_cases = ['CIV-2024-038', 'SYN-2025-002', 'SYN-2025-001', 'FIR-74/2012/SW']
        ...
        query = text("""
            ...
            event_max_time AS (
                SELECT el.case_id, MAX(e.tx_start) as max_event_tx, MAX(upper(e.occurred_at)) as max_event_occ
        ...
        rows = res.fetchall()
        print(f"=== Aggregated Query Top 10 cases ===")
        for r in rows:
            print(dict(r._mapping))

if __name__ == "__main__":
    asyncio.run(test_case_aggregation())
