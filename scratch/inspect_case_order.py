...
async def main():
    ...
    # Query enriched cases order as returned by the API default query
    rows = await conn.fetch("""
        ...
        SELECT case_id, case_number, title, priority, last_activity_at, provenance
        FROM enriched_cases
        ORDER BY 
            (CASE WHEN provenance = 'GOLDEN' THEN 0 ELSE 1 END) ASC,
            last_activity_at DESC;
    """)
    
    print(f"Total cases found: {len(rows)}\n")
    for i, r in enumerate(rows[:15], 1):
        print(f"{i:2d}. [{r['case_number']}] {r['title'][:40]:<40} | Activity: {r['last_activity_at']}")
