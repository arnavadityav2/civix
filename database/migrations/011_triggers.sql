...
BEGIN
    ...
    -- Base case: if we are explicitly closing the row (e.g., via the recursive UPDATE below),
    -- allow the update to proceed.
    IF NEW.tx_end IS NOT NULL AND OLD.tx_end IS NULL THEN
        RETURN NEW;
    END IF;

    -- Step 1: Close the old row on disk (this triggers a recursive call which hits the base case above)
    ...
    -- (This now succeeds because the old row's tx_end is no longer NULL, freeing the partial unique index)
